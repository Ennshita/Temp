...
val_dataloader = dict(
    dataset=dict(
        ...
        data_root=data_root,
        metainfo=metainfo,
        ann_file='../../dataset/json_labels/vipcup.json',
        data_prefix=dict(img='../../dataset/vipcupnight2k/images/')
        ))
test_dataloader = val_dataloader

val_evaluator = dict(  # Validation evaluator config
    type='CocoMetric',  # The coco metric used to evaluate AR, AP, and mAP for detection and instance segmentation
    ann_file='../../dataset/json_labels/vipcup.json',  # Annotation file path
    metric=['bbox'],  # Metrics to be evaluated, `bbox` for detection and `segm` for instance segmentation
    format_only=True,
    outfile_prefix='./work_dirs/infer_syn_vis_fis_vipcup')
